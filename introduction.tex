...
to apply the traditional mocking approach to it: the peer
in \cref{fig:architecture} has been replaced with a mocked peer that does
not implement consensus nor gossiping, but just executes the transaction
requests with an actual application. The advantage is that
testcases can be executed must faster (minutes) than against an actual peer
of an actual blockchain (hours),
that must abide to consensus and create blocks slowly, at regular intervals.
The limit is that mocking only allows one to test the application
(\cref{fig:architecture}), since the other components are mocked.
...
\item the mocking strategy of the peer, that makes the execution of the
  testcases faster and allows their use for continuous integration.
\end{itemize}
...
and Hotmoka peers. \Cref{sec:mocking} describes the implementation
of a mocked Hotmoka peer, that still uses an actual application to run
smart contract transactions. \Cref{sec:experiments} reports experiments with
the testing of Takamaka smart contracts against different implementations
of the API, including the mocked one. \Cref{sec:conclusion} concludes.
