...
run inside a blockchain and modify the state
of the application implemented by the blockchain.
For instance, Ethereum smart contracts are typically written in
Solidity, can be installed in blockchain and their
...
as a sequence of transactions that start from an initial fixture.
...
\begin{figure*}[t]
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [fill=verylightblue,thick] (-6.2,-0.8) rectangle +(2.2,1.2);
      \draw (-5.1,0) node {client(s)};
      \draw (-5.1,-0.4) node {(\eg testcases)};
  ...
  \caption{The architecture of a blockchain peer.
    ...
    software tools that connect to the peer through an
    abstract blockchain interface (ABI)
    and run transactions to install smart contracts
    and run their methods. Testcases are an example of clients.}\label{fig:architecture}
\end{figure*}
...
eventually stored in its database of blocks. Miners receive a challenge to solve
(for instance, the mimimization of the hash of a new block) and provide
...
But they can also be testcases, that execute testing scenarios on the peer,
which is the perspective taken in this article.
...
UTXOs, while the Ethereum application has transactions that execute
arbitrary EVM bytecode. Transaction requests are first parked inside a
mempool \ie a queue from where they are eventually extracted for execution.
...
in the case of Ethereum. We name that interface as the peer's ABI
(abstract blockchain interface).

Testcases can be seen as clients that use the ABI to simulate
...
for modifications to the blockchain peer, miners or application, even for
a complete replacement of the peer implementation. The target of the testing activity are
...
This has been possible because the ABI between clients and peer
...
in \cref{fig:architecture} has been replaced with a stubbed one that does
...
%
\begin{itemize}
\item the description of the ABI between the clients and an
  Hotmoka blockchain peer, meant to allow the execution of object-oriented
  code in blockchain;
\item the description of how that single ABI allows one to
...
\end{itemize}
%
The use of a single ABI, for local, remote, actual or stubbed peers, allows one
...
in the Takamaka subset of Java used by Hotmoka and of two testcases for that
smart contract. \Cref{sec:interface} presents the ABI between clients
and Hotmoka peers. \Cref{sec:stubbing} describes the implementation
of a stubbed Hotmoka peer.
\Cref{sec:adapting} shows how the ABI between clients and peers can be adapted to
a remote peer, potentially running in another machine and reachable through
...
of the ABI, including the stubbed one, both local and remote.
