...
and Ethereum-like blockchains, that still cover a majority of the
installed smart contracts.
Pytest~\citep{pytest} is a Python test suite for Ethereum smart contracts.
Foundry~\citep{foundry} is a Rust framework that includes a testing suite
where Solidity contracts create the fixture and run the tests.
...
they use the Web3 API to install the contracts in blockchain
...
Just to provide a few examples, there are testing frameworks
...
since dynamic sequences of behaviors are tested, rather than
...
That is, tests check consistency in the output rather than the exact output.
...
Fuzzing has also been used to generate and strengthen testcases for
smart contracts~\citep{LahamiKMMR24,WongZNLCYLLW24}.
